...
class OtpService {
  ...
  storeOtp = async (user: string, otp: number, type: string): Promise<IOtpDocument> => {
    return await OtpModel.create({ user, otp, type });
  };

  removeOtp = async (user: string): Promise<void> => {
    await OtpModel.deleteOne({ user });
  };

  verifyOtp = async (user: string, otp: number, type: string): Promise<string> => {
